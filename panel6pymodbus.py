...
class panel6():
    ...
    def p6_dc1_real_array(self):
        ...
        if (p6dc1list.registers[1] & 256) == 0:
            self.p6dc1list_voltage = round(
                -(float(p6dc1list.registers[0])) * (10.0 ** (float(p6dc1list.registers[1]) - 3)), 2)
        ...
        if (p6dc1list.registers[5] & 256) == 0:
            self.p6dc1list_power = round((((p6dc1list.registers[4]) * (10.0 ** (p6dc1list.registers[5] - 3))) / 1000.0),
                                         2)
        else:
            self.p6dc1list_power = round(
                (((-1.0 * float(p6dc1list.registers[4])) * (10.0 ** (float(p6dc1list.registers[5] - 259)))) / 1000.0),
                2)
        # print("p6_dc1 Power: " +str(self.p6dc1list_power))
        # Positive Energy
        self.p6dc1list_PositiveEnergy = (65536.0 * float(p6dc1list.registers[6])) + (
        float(p6dc1list.registers[7]) / 100.0)
    ...
    def p6_dc2_real_array(self):
        ...
        if (p6dc2list.registers[5] & 128) == 0:
            ...
            self.p6dc2list_power = ((-1.0 * float(p6dc2list.registers[4])) * (
            10.0 ** (float(p6dc2list.registers[5] - 259)))) / 1000.0
        # print("p6_dc2 Power: " +str(self.p6dc2list_power))
        # Positive Energy
        self.p6dc2list_PositiveEnergy = (65536.0 * float(p6dc2list.registers[6])) + (
        float(p6dc2list.registers[7]) / 100.0)
