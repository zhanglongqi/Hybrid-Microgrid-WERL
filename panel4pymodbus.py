...
class panel4():
    ...
    def p4_dc1_real_array(self):
        ...
        if (p4dc1list.registers[1] & 256) == 0:
            self.p4dc1list_voltage = round(
                -(float(p4dc1list.registers[0])) * (10.0 ** (float(p4dc1list.registers[1]) - 3)), 2)
        ...
        if (p4dc1list.registers[5] & 256) == 0:
            self.p4dc1list_power = round((((p4dc1list.registers[4]) * (10.0 ** (p4dc1list.registers[5] - 3))) / 1000.0),
                                         2)
        else:
            self.p4dc1list_power = round(
                (((-1.0 * float(p4dc1list.registers[4])) * (10.0 ** (float(p4dc1list.registers[5] - 259)))) / 1000.0),
                2)
        # print("p4_dc1 Power: " +str(self.p4dc1list_power))
        # Positive Energy
        self.p4dc1list_PositiveEnergy = (65536.0 * float(p4dc1list.registers[6])) + (
        float(p4dc1list.registers[7]) / 100.0)
    ...
    def p4_dc2_real_array(self):
        ...
        if p4dc2list.registers[5] == 0:
            ...
            self.p4dc2list_power = ((-1.0 * float(p4dc2list.registers[4])) * (
            10.0 ** (float(p4dc2list.registers[5] - 259)))) / 1000.0
        # print("p4_dc2 Power: " +str(self.p4dc2list_power))
        # Positive Energy
        self.p4dc2list_PositiveEnergy = (65536.0 * float(p4dc2list.registers[6])) + (
        float(p4dc2list.registers[7]) / 100.0)
