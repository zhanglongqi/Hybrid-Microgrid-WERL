...
class panel7():
    ...
    def p7_dc1_real_array(self):               
        ...
        if (p7dc1list.registers[1] & 256) == 0:
            self.p7dc1list_voltage = round(-(float(p7dc1list.registers[0])) * (10.0** (float(p7dc1list.registers[1])-3)), 2)
        ...
        if (p7dc1list.registers[5] & 256) == 0:
            self.p7dc1list_power = round((((p7dc1list.registers[4]) * (10.0** (p7dc1list.registers[5]-3)) )/1000.0 ), 2)           
        else:
            self.p7dc1list_power = round((((-1.0*float(p7dc1list.registers[4])) * (10.0**(float(p7dc1list.registers[5]-259))) )/1000.0), 2)
        #print("p7_dc1 Power: " +str(self.p7dc1list_power))            
        #Positive Energy
        self.p7dc1list_PositiveEnergy = (65536.0*float(p7dc1list.registers[6])) + (float(p7dc1list.registers[7])/100.0)
...
#-------------------------------------------------------------------------------------------------------------------------------#  
    ...
    def p7_dc2_real_array(self):               
        ...
        if p7dc2list.registers[5] == 0:
            ...
            self.p7dc2list_power = ( (-1.0*float(p7dc2list.registers[4])) * (10.0**(float(p7dc2list.registers[5]-259))) )/1000.0
        #print("p7_dc2 Power: " +str(self.p7dc2list_power))            
        #Positive Energy
        self.p7dc2list_PositiveEnergy = (65536.0*float(p7dc2list.registers[6])) + (float(p7dc2list.registers[7])/100.0)
