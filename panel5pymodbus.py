...
class panel5():                          
    ...
    def p5_dc1_real_array(self):               
        ...
        if (p5dc1list.registers[1] & 256) == 0:
            self.p5dc1list_voltage = round(-(float(p5dc1list.registers[0])) * (10.0** (float(p5dc1list.registers[1])-3)), 2)
        ...
        if (p5dc1list.registers[5] & 256) == 0:
            self.p5dc1list_power = round((((p5dc1list.registers[4]) * (10.0** (p5dc1list.registers[5]-3)) )/1000.0 ), 2)           
        else:
            self.p5dc1list_power = round((((-1.0*float(p5dc1list.registers[4])) * (10.0**(float(p5dc1list.registers[5]-259))) )/1000.0), 2)
...
#------------------------------------------------------------------------------------------------------------------------------#
    ...
    def p5_dc2_real_array(self):               
        ...
        if (p5dc2list.registers[1] & 256) == 0:
            self.p5dc2list_voltage = round(-(float(p5dc2list.registers[0])) * (10.0** (float(p5dc2list.registers[1])-3)), 2)
            #print('voltage for reg=0')
        else:
        ...
        if (p5dc2list.registers[5] & 256) == 0:
            self.p5dc2list_power = round((((p5dc2list.registers[4]) * (10.0** (p5dc2list.registers[5]-3)) )/1000.0 ), 2)           
        else:
            self.p5dc2list_power = round((((-1.0*float(p5dc2list.registers[4])) * (10.0**(float(p5dc2list.registers[5]-259))) )/1000.0), 2)
        #print("p5_dc2 Power: " +str(self.p5dc2list_power))   
        #Positive Energy
        self.p5dc2list_PositiveEnergy = (65536.0*float(p5dc2list.registers[6])) + (float(p5dc2list.registers[7])/100.0)
